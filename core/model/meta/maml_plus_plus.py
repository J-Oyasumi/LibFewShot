...
class MAMLPlusPlus(MetaModel):
    def __init__(
        ...
        **kwargs
        ...
        self.classifier = MAMLPlusPlusLayer(
            feat_dim, way_num=self.way_num, use_layer_norm=use_layer_norm
        )
        ...
        self.enable_inner_loop_optimizable_bn_params = inner_param.get(
        ...
        self.first_order_to_second_order_epoch = inner_param.get(
    ...
    def set_forward_adaptation(self, support_set, support_target):
        ...
        num_steps = (
            self.inner_param["train_iter"] if self.training else self.inner_param["test_iter"]
        )
        ...
        for i in range(num_steps):
            ...
            create_graph = self.second_order and not self.is_first_order_epoch(
                getattr(self, "_current_epoch", 0)
            )

            grad = torch.autograd.grad(
                loss, model_parameters, create_graph=create_graph, allow_unused=True
            )
    ...
    def set_current_epoch(self, epoch):
        ...
        if (
            self.first_order_to_second_order_epoch > 0
